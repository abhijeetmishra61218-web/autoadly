...
def _has_real_adbot(adbots):
    """A customer's slot list can be non-empty while every slot is an empty,
    released placeholder (ad_account_id None) — e.g. right after their plan
    expired and free_customer_slots() cleared the accounts but kept the
    slots so a resubscribe reuses the same name/bio/photo. `if not adbots`
    alone doesn't catch that; check for at least one slot with a real
    account attached."""
    return any(a.get("ad_account_id") is not None for a in adbots)

def _subscription_active(user_id):
    import time as _time
    sub = store.get_subscription(user_id)
    return bool(sub) and sub.get("expiry", 0) > _time.time()

@router.callback_query(F.data == "myad:set_ad")
async def cb_set_ad_start(callback: CallbackQuery):
    user_id = callback.from_user.id
    if not _subscription_active(user_id):
        await callback.answer("Your subscription has expired. Tap Buy Ad Bot to renew and get set up again.", show_alert=True)
        return
    adbots = store.get_customer_adbots(user_id)
    if not _has_real_adbot(adbots):
...
async def _show_account_picker(chat_id, msg_id, user_id):
    adbots = store.get_customer_adbots(user_id)
    # Only list slots that actually have an account attached — an expired/
    # released slot keeps its name for reuse later, but must never render
    # as a pickable option since there's nothing behind it.
    rows = [
        [{"text": store.slot_display_name(bot, i), "callback_data": f"adwiz:acct:{i}"}]
        for i, bot in enumerate(adbots) if bot.get("ad_account_id") is not None
    ]
    rows.append([{"text": "Cancel", "callback_data": "myadbot:open"}])
...
async def cb_pick_account(callback: CallbackQuery):
    ...
    if idx >= len(adbots) or adbots[idx].get("ad_account_id") is None:
        await callback.answer("That Ad Bot Account isn't available anymore. Tap Buy Ad Bot to set up again.", show_alert=True)
        return
    bot = adbots[idx]
...
async def cb_pick_list(callback: CallbackQuery):
    ...
    # TEMP OVERRIDE (2026-08-19): "Only High Quality Groups", "Only High Quality
    # Forums" and "Only OFM Groups & Forums" presets are empty shells with no
    # marketplaces linked to them. Picking one silently creates an ad that never
    # posts (engine.run_advertisement_loop sees an empty list and just returns).
    # Until those lists are populated for real, force every preset-list pick to
    # land on "All Groups & Forums" (which is fully populated) so no new client
    # can hit the same silent-failure. This does NOT affect the "custom" /
    # folder-link flow, which builds its own list from real groups.
    safe_list_id = await db.get_or_create_list("All Groups & Forums")
    if list_id != safe_list_id:
        logger.warning(
            f"adwizard: overriding list pick {list_id} -> safe list {safe_list_id} "
            f"for user {user_id} (empty-preset-list guard)"
        )
