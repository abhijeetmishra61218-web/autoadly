...
-- Permanent, never-pruned running totals. post_logs itself only ever
-- holds a short recent window (see backup_system.py's 24h retention)
-- for operational/debugging use, so raw post_logs alone cannot answer
-- "today / last 7 days / all-time" style questions honestly. These two
-- tables are the actual source of truth for that: one row per
-- (day, marketplace) / (day, ad_account) pair, incremented in
-- log_success(), never deleted.
CREATE TABLE IF NOT EXISTS daily_marketplace_stats (
    day TEXT,
    marketplace_id INTEGER,
    post_count INTEGER DEFAULT 0,
    PRIMARY KEY (day, marketplace_id)
);
...
async def log_success(ad_account_id: int, marketplace_id: int, message_link: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            (ad_account_id, marketplace_id, ts, message_link)
        )
        await db.execute(
            "INSERT INTO daily_marketplace_stats (day, marketplace_id, post_count) VALUES (?, ?, 1) "
            "ON CONFLICT(day, marketplace_id) DO UPDATE SET post_count = post_count + 1",
            (day, marketplace_id)
        )
        await db.execute(
            "INSERT INTO daily_account_stats (day, ad_account_id, post_count) VALUES (?, ?, 1) "
            "ON CONFLICT(day, ad_account_id) DO UPDATE SET post_count = post_count + 1",
            (day, ad_account_id)
        )
        await db.commit()
...
async def cleanup_old_logs():
    """NOTE: intentionally no longer called anywhere (see engine.py's
    cleanup_loop). This ran every 60s with a 900s cutoff, pruning
    post_logs down to a rolling ~15-minute window — which completely
    overrode backup_system.py's intended 24h retention and made
    /teststats-style "today/week/all-time" queries meaningless (they were
    all reading the same ~15 minutes of data). Real totals now live in
    daily_marketplace_stats / daily_account_stats instead, which are never
    pruned. Left defined, unused, in case a short debug-window prune is
    ever wanted again deliberately."""
...
async def get_occupied_ad_accounts():
    """Every Ad Bot Account currently assigned to a customer (status='occupied') —
       used by the full concurrent restriction sweep."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
