...
async def _run_with_restart(ad):
    ...
    try:
        ...
        # Always release the id, including the "empty marketplace list" clean-exit
        # case in run_advertisement_loop. Otherwise, if the DB's marketplace_list_id
        # for this ad is fixed later while status stays 'active', watch_for_new_ads()
        # will never retry it (it only starts ids not already in _running_ad_ids) —
        # the ad would need a full bot restart to actually start posting again.
        _running_ad_ids.discard(ad_id)
...
async def start_engine():
    ...
    async def cleanup_loop():
        while True:
            # cleanup_old_logs() used to run here every 60s with a 900s
            # cutoff, pruning post_logs to a rolling ~15-minute window —
            # that fought backup_system.py's 24h retention and made any
            # "today/week/all-time" stat meaningless. Real running totals
            # now come from daily_marketplace_stats/daily_account_stats
            # instead (see database.log_success), which are never pruned,
            # so this loop has nothing left to do. Left in place, inert,
            # in case a deliberate short-window prune is wanted later.
