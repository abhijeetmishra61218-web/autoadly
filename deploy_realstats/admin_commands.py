...
async def cmd_force_assign(message: Message):
    """Admin-only: /force <account_id> <@username>
    Directly assigns a specific Ad Bot Account to a specific customer,
    with NO @SpamBot check — for manually fixing a free-but-unassigned
    account, or moving a number off one person's slot onto another's.
    If the account is currently sitting in someone else's slot, it's
    pulled off them first (that customer simply loses that number)."""
    if not store.is_admin(message.from_user.id):
        return
    parts = message.text.split()
    if len(parts) != 3 or not parts[2].startswith("@"):
        await message.reply("Usage: /force <account_id> @username")
        return

    try:
        account_id = int(parts[1])
    except ValueError:
        await message.reply("Usage: /force <account_id> @username — account_id must be a number.")
        return
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        ...
        cursor = await conn.execute("SELECT id, phone, status FROM ad_accounts WHERE id = ?", (account_id,))
        account = await cursor.fetchone()
    if not account:
        await message.reply(f"No Ad Bot Account found with ID {account_id}.")
        return

    prev_owner = store.unassign_account_everywhere(account_id)

    existing_ad = await db.get_active_ad_for_account(account_id)
    if existing_ad:
        await db.stop_advertisement(existing_ad["id"])

    store.add_customer_adbot(target_uid, None, account_id)
    await db.mark_ad_account_status(account_id, "occupied")

    lines = [
        f"Account ID {account_id} ({account['phone']}) force-assigned to @{username}. "
        f"No @SpamBot check was run.",
    ]
    if prev_owner:
        prev_uid_str, prev_slot_name = prev_owner
        prev_username = store.get_username_by_uid(prev_uid_str)
        prev_label = f"@{prev_username}" if prev_username else f"user_id={prev_uid_str}"
        lines.append(f"It was previously assigned to {prev_label} ({prev_slot_name}) — that slot has been cleared.")
    if existing_ad:
        lines.append("Its running ad was stopped before reassigning.")
    if account["status"] not in ("free", "occupied"):
        lines.append(f"Note: this account's prior status was '{account['status']}' — now forced to 'occupied' regardless.")
    ...
    try:
        await raw_api.send_message(
            target_uid,
            "You've been given an Ad Bot Account by the admin. Tap /set to set up your ad.",
            [],
        )
    except Exception as e:
        print(f"[cmd_force_assign] could not notify user {target_uid}: {e}")

def _is_owner(user_id):
    """Strictly the owner_id in admins.json — NOT cofounders/other admins.
    Used to gate features still being tested before they go anywhere
    customer-facing (public stats channel, leaderboard, etc.)."""
    admins = store.load_admins()
    return user_id == admins.get("owner_id")

@router.message(Command("teststats"))
async def cmd_test_stats(message: Message):
    """Owner-only, TESTING ONLY — preview of what a public stats post could
    look like: total ads posted today / last 7 days / all-time, and across
    how many distinct marketplaces. Nothing here is shown to customers or
    posted anywhere automatically — this just renders the numbers so you
    can see what they'd look like before deciding to make it public.

    Reads from daily_marketplace_stats, not post_logs — post_logs only
    ever holds a short recent window (see backup_system.py), so it can't
    honestly answer "today/week/all-time" on its own."""
    if not _is_owner(message.from_user.id):
        return

    today_str = time.strftime("%Y-%m-%d", time.gmtime(time.time()))
    week_days = [time.strftime("%Y-%m-%d", time.gmtime(time.time() - i * 86400)) for i in range(7)]
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        ...
        cur = await conn.execute(
            "SELECT COALESCE(SUM(post_count),0) AS c, COUNT(DISTINCT marketplace_id) AS m "
            "FROM daily_marketplace_stats WHERE day = ?",
            (today_str,),
        )
        today = await cur.fetchone()
        cur = await conn.execute(
            "SELECT COALESCE(SUM(post_count),0) AS c, COUNT(DISTINCT marketplace_id) AS m "
            f"FROM daily_marketplace_stats WHERE day IN ({placeholders})",
            week_days,
        )
    ...
    await message.reply(
        "<b>📊 AutoAdly Stats — preview (owner-only test, not public yet)</b>\n\n"
        f"Today: <b>{today['c']}</b> ads posted across <b>{today['m']}</b> marketplaces\n"
        f"Last 7 days: <b>{week['c']}</b> ads posted across <b>{week['m']}</b> marketplaces\n"
        f"All-time: <b>{alltime['c']}</b> ads posted across <b>{alltime['m']}</b> marketplaces",
        parse_mode="HTML",
    )

@router.message(Command("testleaderboard"))
async def cmd_test_leaderboard(message: Message):
    """Owner-only, TESTING ONLY — preview of an anonymized top-advertisers
    leaderboard by post volume over the last 7 days. No usernames or any
    identifying info shown, just 'Advertiser #N' + post count, exactly what
    a public trust-building post would look like. Nothing here is shown to
    customers or posted anywhere automatically.

    Reads from daily_account_stats, not post_logs, for the same reason as
    /teststats above — post_logs doesn't retain a real 7-day window."""
    if not _is_owner(message.from_user.id):
        return
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        conn.row_factory = aiosqlite.Row
        cur = await conn.execute(
            "SELECT ad_account_id, SUM(post_count) AS c FROM daily_account_stats "
            f"WHERE day IN ({placeholders}) GROUP BY ad_account_id",
            week_days,
        )
        rows = await cur.fetchall()

    if not rows:
        await message.reply("No posts logged in the last 7 days yet.")
        return
    ...
    for uid_str, bots in all_adbots.items():
        for bot in bots:
            if bot.get("ad_account_id") is not None:
                owner_by_account[bot["ad_account_id"]] = uid_str

    per_customer = {}
    for r in rows:
        uid_str = owner_by_account.get(r["ad_account_id"])
        if uid_str is None:
            continue  # account not currently tied to any customer (freed/orphaned log entries)
        per_customer[uid_str] = per_customer.get(uid_str, 0) + r["c"]

    if not per_customer:
        await message.reply("Posts were logged, but none map to a currently-assigned customer.")
        return

    ranked = sorted(per_customer.items(), key=lambda x: x[1], reverse=True)[:10]

    lines = ["<b>🏆 Top Advertisers — last 7 days, preview (owner-only test, not public yet)</b>", ""]
    for i, (uid_str, count) in enumerate(ranked, 1):
        lines.append(f"Advertiser #{i} — {count} ads posted")
