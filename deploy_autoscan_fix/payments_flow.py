...
async def cb_pick_crypto(callback: CallbackQuery):
    ...
    # NOTE: background auto-scan intentionally NOT started here anymore.
    # It used to silently match ANY transaction landing on the shared address
    # (from any customer, for any order) purely by amount, which could steal
    # a different customer's real, valid transaction hash before they pasted
    # it themselves -> falsely "already used". Orders are now completed only
    # through the customer's own manual hash paste in on_hash_text(), which
    # already does the correct check order: hash not used -> tx really
    # happened (amount + receiver address match) -> tx not older than the
    # payment window -> mark hash used. _auto_scan() is left defined below,
    # unused, in case a safer, per-order-tagged version is built later.
