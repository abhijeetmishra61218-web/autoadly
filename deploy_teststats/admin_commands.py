...
async def cmd_test_stats(message: Message):
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        ...
        cur = await conn.execute(
            "SELECT COUNT(*) AS c, COUNT(DISTINCT marketplace_id) AS m FROM post_logs WHERE posted_at >= ?",
            (day_start,),
        )
        ...
        cur = await conn.execute("SELECT COUNT(*) AS c FROM post_logs")
    ...
    await message.reply(
        ...
        f"All-time: <b>{alltime['c']}</b> ads posted",
        parse_mode="HTML",
    )
...
async def cmd_test_leaderboard(message: Message):
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        conn.row_factory = aiosqlite.Row
        cur = await conn.execute(
            "SELECT ad_account_id, COUNT(*) AS c FROM post_logs WHERE posted_at >= ? GROUP BY ad_account_id",
            (since,),
        )
